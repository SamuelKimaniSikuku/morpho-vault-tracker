  ...
  return <div className="simple-vault-list">
    ...
    <ul>{vaults.map(v => {
      ...
      return <li key={key} className="simple-vault-item">
        ...
        <div className="simple-vault-metric"><span className="mobile-metric-label">Liquidity</span><strong title={live?.liquidityUsd == null ? "The source does not report available withdrawal liquidity" : "Available withdrawal liquidity reported by the source"}>{formatMoney(live?.liquidityUsd)}</strong></div>
