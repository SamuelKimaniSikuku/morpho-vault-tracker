...
export interface VaultFilters { protocol: Protocol | "all"; network: string; asset: string; category?: "all" | "variable" | "fixed" }
export const ALL_FILTERS: VaultFilters = { protocol: "all", network: "all", asset: "all", category: "all" };
...
export function filterVaults<T extends WatchedVault>(vaults: T[], filters: VaultFilters): T[] {
  return vaults.filter(v => (filters.protocol === "all" || v.protocol === filters.protocol)
    && (!filters.category || filters.category === "all" || (filters.category === "fixed" ? !!v.fixedTerm : !v.fixedTerm))
