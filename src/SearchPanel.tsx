...
const PAGE_SIZE = 8;
export function SearchPanel({ query, onQuery, watchlist, onAdd, onDetails, onClose }: { query: string; onQuery: (query: string) => void; watchlist: WatchedVault[]; onAdd: (v: VaultSummary) => void; onDetails: (v: VaultSummary) => void; onClose: () => void }) {
  ...
  return <section className="search-panel vault-search" aria-label="Find and add vaults">
    <div className="inline-panel-heading"><h2>Add a vault</h2><button className="text-button" onClick={onClose}>Done</button></div>
    <label className="sr-only" htmlFor="vault-search">Search vaults by name or coin</label>
    <div className="search-tools">
      <div className="search-field"><Icon name="search" /><input id="vault-search" type="search" autoComplete="off" maxLength={1300} placeholder="Search a vault or coin, like USDC" value={query} onChange={e => onQuery(e.target.value)} />{query && <button className="icon-button" aria-label="Clear search" onClick={() => onQuery("")}><Icon name="close" /></button>}</div>
    ...
    {(report || busy) && <div className="search-results">
      ...
      {filtered.length > 0 && <><div className="search-column-headings" aria-hidden="true"><span>Vault</span><span>Yearly rate</span><span>{filtered.some(v => v.fixedTerm) ? "Deposits / liquidity" : "Total deposits (TVL)"}</span><span>Liquidity</span><span /></div>
      <ul className="result-list">{filtered.slice(0, limit).map(v => {
        ...
        return <li className="result-row" key={vaultKey(v)}>
          ...
          <div className="search-metric"><span className="search-mobile-label">Liquidity</span><strong title={v.liquidityUsd == null ? "Not reported by this source" : "Available withdrawal liquidity"}>{formatMoney(v.liquidityUsd)}</strong></div>
      ...
      <p className="search-footnote meta">Liquidity is what the source reports as available to withdraw. — means not reported.</p></>}
