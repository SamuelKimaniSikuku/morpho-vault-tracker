...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="dashboard">
      ...
      <div className="page-heading"><div><h1>{view === "watchlist" ? "Your watchlist." : view === "fixed" ? "Fixed Vaults." : "Explore vaults."}</h1><p>{view === "watchlist" ? "Watch yield changes. Know what needs a closer look." : view === "fixed" ? "Morpho, Pendle, and Spectra on Ethereum and Base." : "Discover reported yields, total deposits, and market moves."}</p></div><button className="button" disabled={checking || (view === "watchlist" && !watchlist.length)} onClick={view === "watchlist" ? refresh : view === "fixed" ? () => setFixedRevision(n => n + 1) : () => setExploreRevision(n => n + 1)}><Icon name="refresh" className={checking ? "spinning" : ""} />{checking ? "Checking…" : "Check now"}</button></div>
