...
export function NewsPanel({ watched, onAdd, onDetails, now }: {
  ...
  return <section className="vault-news" aria-label="Vault news">
    ...
    {mode === "updates" ? <section aria-label="Latest vault updates">
      <div className="news-list-heading"><div><h2>Rate changes</h2><p className="meta">Changes reported by DeFiLlama for tracked vaults and lending pools.</p></div><div className="segmented" role="group" aria-label="News time period"><button className={window === "1d" ? "active" : ""} aria-pressed={window === "1d"} onClick={() => setWindow("1d")}>24h</button><button className={window === "7d" ? "active" : ""} aria-pressed={window === "7d"} onClick={() => setWindow("7d")}>7 days</button></div></div>
      ...
      <p className="table-note">Among tracked vaults and lending pools with at least $50,000 in deposits. {mode === "yield" ? "Includes APY from 0% to 100%; APR and unconfirmed rate types are excluded. " : ""}— means not reported. Larger deposits or higher yield do not mean lower risk.</p>
    </section>}
  </section>;
}
